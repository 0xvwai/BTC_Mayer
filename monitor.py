...
def run_monitor():
    ...
    try:
        ...
        report = (
            ...
            f"Value: `{m_str}`  →  {m_status}\n"
            f"```\n"
            f"< 0.8  📉 Undervalued  → Strong DCA\n"
            f"0.8–1.0  🟢 Below MA200  → DCA zone\n"
            f"1.0–1.8  ✅ Normal      → Standard DCA\n"
            f"1.8–2.4  📈 Elevated    → Reduce size\n"
            f"> 2.4  🚨 Overheated  → Pause DCA\n"
            f"```\n\n"

            f"⛓️ *MVRV Ratio* (Market Cap / Realized Cap)\n"
            f"Value: `{mv_str}`  →  {mv_status}\n"
            f"```\n"
            f"< 1.0  💎 Bottom zone  → Aggressive DCA\n"
            f"1.0–2.0  🟢 Accumulate  → DCA zone\n"
            f"2.0–3.0  🟡 Caution     → Reduce size\n"
            f"> 3.0  🚨 Cycle top   → Pause DCA\n"
            f"```\n"
