...
def cm_fetch(metric):
    ...
    url = f"{CM_BASE}?assets=btc&metrics={metric}&frequency=1d&page_size=5"
    try:
        data = requests.get(url, headers=CM_HDR, timeout=15).json().get("data", [])
        for row in reversed(data):
            if row.get(metric) is not None:
                return float(row[metric]), row.get("time", "")[:10]
    except Exception as e:
        print(f"CoinMetrics error ({metric}): {e}")
    return None, None
